...
server.ts        Express + ws server (in-memory state, no database)
```
...
|---|---|---|
| `orders` | `{ id, items, tableNum, status, createdAt }` | All orders |
...
The candidate must design and implement the required endpoints, including appropriate HTTP status codes and response bodies.
...
# Server (port 4001, hot-reloads on file changes)
npm run server
